...
class UserRoomDatabaseTest {
    ...
    fun `Room_INSERT_테스트`() {
        ...
        val test = userDao.getUsers().test()
    ...
    fun `Room_UPDATE_테스트`() {
        ...
        val user = userDao.getUsers().blockingGet().first()
        ...
        val result = userDao.getUsers().blockingGet()
        Log.d("data", "result => ${result[0].name}")
        Log.d("data", "result => ${result[0].phoneNumber}")

//        test.assertValue { it[0].name == data.name }
        userDao.getUsers().test().assertValue { it[0].name == user.name }
    }
}